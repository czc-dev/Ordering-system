...
ExamSet.create!(name: '生化学的検査').tap do |exam_set|
  exam_set.exam_items << ExamItem.where(id: 1..55)
end

ExamSet.create!(name: '尿・糞便検査').tap do |exam_set|
  exam_set.exam_items << ExamItem.where(id: 56..62)
end

ExamSet.create!(name: '血液学的検査').tap do |exam_set|
  exam_set.exam_items << ExamItem.where(id: 63..78)
end

ExamSet.create!(name: '耐糖能関連検査').tap do |exam_set|
  exam_set.exam_items << ExamItem.where(id: 79..91)
end

ExamSet.create!(name: '免疫学的検査').tap do |exam_set|
  exam_set.exam_items << ExamItem.where(id: 92..121)
end

ExamSet.create!(name: 'ウイルス・感染症関連検査').tap do |exam_set|
  exam_set.exam_items << ExamItem.where(id: 122..142)
end

ExamSet.create!(name: '腫瘍マーカー').tap do |exam_set|
  exam_set.exam_items << ExamItem.where(id: 143..158)
end

ExamSet.create!(name: '甲状腺関連検査').tap do |exam_set|
  exam_set.exam_items << ExamItem.where(id: 159..170)
end

ExamSet.create!(name: '内分泌学的検査').tap do |exam_set|
  exam_set.exam_items << ExamItem.where(id: 171..181)
end

ExamSet.create!(name: '薬物血中濃度検査').tap do |exam_set|
  exam_set.exam_items << ExamItem.where(id: 171..181)
end

ExamSet.create!(name: '血算5項目セット').tap do |exam_set|
...
ExamSet.create!(name: '急性期:若里セット').tap do |exam_set|
...
ExamSet.create!(name: '定期セット').tap do |exam_set|
  exam_set.exam_items << ExamItem.find_by(abbreviation: 'ALB')
  exam_set.exam_items << ExamItem.find_by(abbreviation: 'GOT')
  exam_set.exam_items << ExamItem.find_by(abbreviation: 'GPT')
  exam_set.exam_items << ExamItem.find_by(abbreviation: 'BUN')
...
ExamSet.create!(name: '脂質セット').tap do |exam_set|
...
ExamSet.create!(name: '心不全セット').tap do |exam_set|
  exam_set.exam_items << ExamItem.find_by(abbreviation: 'UA')
end

ExamSet.create!(name: '糖尿病セット').tap do |exam_set|
  exam_set.exam_items << ExamItem.find_by(abbreviation: 'HBA1C')
end

ExamSet.create!(name: '甲機セット').tap do |exam_set|
